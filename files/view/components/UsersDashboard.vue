<template>
    <div id="LinksDashboard">
        ...
        <div id="AddSettingsButton">
            ...
            <a class="btn btn-info" :href="urls.exportUrl" target="_blank">Export {{title}}</a>

            <span id="importSection">
                <form
                    :action="urls.importUrl"
                    method="post"
                    ref="import"
                ...
                >
                    ...
                    <button class="btn btn-warning">Import {{title}}</button>
                </form>
            </span>
        </div>
...
export default {
    ...
    data() {
        return {
            ...
            actions: ["DeleteItem", "EditItem", "CloneItem"],
            showDetails: {
                name: "User",
                column: "name"
            },
            formDetails: [
                ...
                {
                    name: "Role ID",
                    placeholder: "Write User Roled ID",
                    type: "selectbox",
                    show: "show",
                    value: "value",
                    items: [
    ...
    methods: {
        ...
        orderIndex(data) {
            ...
            axios
                .get(
                    `${this.search.searchUrl}?order=${data.column}&orderBy=${data.type}&api=1`
                )
                .then(r => {
                    let data = r.data;
        ...
        store(data) {
            axios
                .post(this.urls.addUrl, data)
                .then(r => {
                    this.localData.gw.unshift(r.data);
                    this.newAlert(
                        "New " + this.title + " Added",
                        `${this.title} ${r.data.name} Was Added Successfully`,
                        true
                    );
        ...
        newAlert(title, message, success) {
            if (success) {
                this.alerts.successMessages.push({
                    title: title,
                    text: message
                });
            } else {
        ...
        PaginationMove(data) {
            axios
                .get(
                    `${data.url}?offset=${data.offset}&perpage=${
                        data.perpage
                    }&search=${
                        this.localData.search.searchPhrase == false
                            ? ""
                            : this.localData.search.searchPhrase
                    }&api=1`
                )
                .then(r => {
                    let data = r.data;
        ...
        update(data) {
            axios
                ...
                .then(r => {
                    ...
                    this.newAlert(
                        "Update " + this.title,
                        `${r.data.Name} Updated Successfully`,
                        true
                    );
